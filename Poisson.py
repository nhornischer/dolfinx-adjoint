...
if __name__ == "__main__":
    ...
    with dolfinx.io.XDMFFile(
        MPI.COMM_WORLD, dir + "/poisson_results.xdmf", "w"
    ) as xdmf:
...
class TestPoisson(unittest.TestCase):
    def test_Poisson_dJdf(self):
        ...
        self.assertTrue(
            np.allclose(graph_.backprop(id(J), id(f)), gradient_df.vector.array[:])
        )
    ...
    def test_Poisson_dJdbc(self):
        ...
        self.assertTrue(
            np.allclose(graph_.backprop(id(J), id(uD_L)), gradient_bc.vector.array[:])
        )
